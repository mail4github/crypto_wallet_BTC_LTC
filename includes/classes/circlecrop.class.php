...
class CircleCrop
{
    ...
    public function __construct($img, $destination_width, $destination_height)
    {
        $this->src_img = $img;
        $this->src_w = imagesx($img);
        $this->src_h = imagesy($img);
        $this->dst_w = $destination_width;//imagesx($img);
        $this->dst_h = $destination_height;//imagesy($img);
    }
    ...
    public function reset()
    {
        ...
        imagecopyresampled(
			...
			0, 0, 
			0, 0, 
			$this->dst_w, $this->dst_h,
			imagesx($this->src_img), imagesy($this->src_img)
		);
        return $this;
    }
