...
$toolbar = array(
	array('name' => '', 'selected' => $data_type, 'style' => 'text-align:left;',
		'buttons' => array(
			array('name' => 'Deposits and withdrawals', 'onclick'=> '$(\'input[name=data_type]\').val(\'HF_in_out\'); document.graph_frm.submit();', 'value' => 'HF_in_out'),
			array('name' => 'Bank and stack', 'onclick'=> '$(\'input[name=data_type]\').val(\'HF_bank_and_stack\'); document.graph_frm.submit();', 'value' => 'HF_bank_and_stack'),
			array('name' => 'Number of transactions', 'onclick'=> '$(\'input[name=data_type]\').val(\'HF_number_of_transactions\'); document.graph_frm.submit();', 'value' => 'HF_number_of_transactions'),
			array('name' => 'Number of players', 'onclick'=> '$(\'input[name=data_type]\').val(\'HF_number_of_players\'); document.graph_frm.submit();', 'value' => 'HF_number_of_players'),
			array('name' => 'Bets by Country', 'onclick'=> '$(\'input[name=data_type]\').val(\'HF_transactions_by_country\'); document.graph_frm.submit();', 'value' => 'HF_transactions_by_country'),
			array('name' => 'Players by Country', 'onclick'=> '$(\'input[name=data_type]\').val(\'HF_players_by_country\'); document.graph_frm.submit();', 'value' => 'HF_players_by_country'),
			array('name' => 'Amount by Country', 'onclick'=> '$(\'input[name=data_type]\').val(\'HF_amount_by_country\'); document.graph_frm.submit();', 'value' => 'HF_amount_by_country'),
			array('name' => 'Average Amount per User by Country', 'onclick'=> '$(\'input[name=data_type]\').val(\'HF_average_amount_per_user_by_country\'); document.graph_frm.submit();', 'value' => 'HF_average_amount_per_user_by_country'),
		)
	),
...
$prefix = '';
$show_negative_values = 0;
switch ( $data_type ) {
	case 'HF_in_out':
		$prefix = DOLLAR_SIGN;
		$caption = 'Deposits and withdrawals';
		$show_negative_values = 1;
	break;
	case 'HF_bank_and_stack':
		$prefix = DOLLAR_SIGN;
		$caption = 'Bank and stack';
		$show_negative_values = 1;
	break;
	case 'HF_number_of_transactions':
		$caption = 'Average number of transactions per hour: ';
	break;
	case 'HF_number_of_players':
		$caption = 'Average number of players per hour: ';
	break;
	case 'HF_transactions_by_country':
		$prefix = 'number of bets: ';
	break;
	case 'HF_players_by_country':
		$prefix = 'number of players: ';
	break;
	case 'HF_amount_by_country':
		$prefix = DOLLAR_SIGN;
	break;
	case 'HF_average_amount_per_user_by_country':
		$prefix = DOLLAR_SIGN;
	break;
}
echo '
...
draw_graph($caption, $data_type, $data_period, $group_by, $date, $prefix, $q_pie, 'graph_container', 'transparent', '', false, $show_negative_values);
