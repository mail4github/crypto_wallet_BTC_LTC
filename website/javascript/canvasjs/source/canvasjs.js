        ...
        for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
            ...
            if (dataPoints.length > 0) {
                ...
                for (i = 0; i < dataPoints.length; i++) {
                    ...
                    var borderThickness = dataSeries.lineThickness;//Math.round(Math.max(1, (barWidth * .15)));
					...
					if ( dataPoints[i].y[0] <= dataPoints[i].y[3] )
						var candle_color = dataSeries.risingColor;
					else
                    ...
                    if (dataSeries.type === "candlestick") {
                        ...
                        drawRect(ctx, x1, Math.min(y1, y4), x2, Math.max(y1, y4), candle_color, borderThickness, candle_color, bevelEnabled, bevelEnabled, false, false, dataSeries.fillOpacity);
						// <--- By me
