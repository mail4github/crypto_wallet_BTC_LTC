...
{
	for(var e=0,t=0;
	t<this.ecBlocks.length;
	t++)e+=this.ecBlocks[t].length;
	return e
}
...
{
	for(this.versionNumber=e,this.alignmentPatternCenters=t,this.ecBlocks=[n,i,r,o],e=0,t=n.ECCodewordsPerBlock,n=n.getECBlocks(),i=0;
	i<n.length;
	i++)r=n[i],e+=r.Count*(r.DataCodewords+t);
...
),this.buildFunctionPattern=function()
{
	...
	for(var n=this.alignmentPatternCenters.length,i=0;
	n>i;
	i++)for(var r=this.alignmentPatternCenters[i]-2,o=0;
	n>o;
	o++)0==i&&(0==o||o==n-1)||i==n-1&&0==o||t.setRegion(this.alignmentPatternCenters[o]-2,r,5,5);
...
{
	for(var t=e.length,n=this.a11,i=this.a12,r=this.a13,o=this.a21,s=this.a22,a=this.a23,h=this.a31,c=this.a32,l=this.a33,f=0;
	t>f;
	f+=2)
{
	var d=e[f],w=e[f+1],u=r*d+a*w+l;
...
,this.transformPoints2=function(e,t)
{
	...
	i++)
{
	var r=e[i],o=t[i],s=this.a13*r+this.a23*o+this.a33;
...
for(var s=Math.abs(n-e),a=Math.abs(i-t),h=-s>>1,c=i>t?1:-1,l=n>e?1:-1,f=0,d=e,o=t;
	d!=n;
	d+=l)
{
	var w=r?o:d,u=r?d:o;
...
{
	var r=this.sizeOfBlackWhiteBlackRun(e,t,n,i),o=1;
...
{
	var n=this.sizeOfBlackWhiteBlackRunBothWays(Math.floor(e.X),Math.floor(e.Y),Math.floor(t.X),Math.floor(t.Y)),i=this.sizeOfBlackWhiteBlackRunBothWays(Math.floor(t.X),Math.floor(t.Y),Math.floor(e.X),Math.floor(e.Y));
...
{
	var t=e.TopLeft,n=e.TopRight;
	e=e.BottomLeft;
	var i=this.calculateModuleSize(t,n,e);
	if(1>i)throw"Error";
	var r=this.computeDimension(t,n,e,i),o=Version.getProvisionalVersionForDimension(r),s=o.DimensionForVersion-7,a=null;
	if(0<o.AlignmentPatternCenters.length)for(s=1-3/s,o=Math.floor(t.X+s*(n.X-t.X+e.X-t.X)),s=Math.floor(t.Y+s*(n.Y-t.Y+e.Y-t.Y));
	;
	)
{
...
function BitMatrix(e,t)
{
	...
	var n=e>>5;
	for(0!=(31&e)&&n++,this.rowSize=n,this.bits=Array(n*t),n=0;
	n<this.bits.length;
	n++)this.bits[n]=0;
...
,this.clear=function()
{
	for(var e=this.bits.length,t=0;
	e>t;
	t++)this.bits[t]=0
}
,this.setRegion=function(e,t,n,i)
{
	...
	for(;
	i>t;
	t++)for(var r=t*this.rowSize,o=e;
	n>o;
	o++)this.bits[r+(o>>5)]|=1<<(31&o)
}
...
,this.readFormatInformation=function()
{
	...
	for(var e=0,t=0;
	6>t;
	t++)e=this.copyBit(t,8,e);
	for(e=this.copyBit(7,8,e),e=this.copyBit(8,8,e),e=this.copyBit(8,7,e),t=5;
	t>=0;
	t--)e=this.copyBit(8,t,e);
	...
	for(var n=this.bitMatrix.Dimension,e=0,i=n-8,t=n-1;
	t>=i;
	t--)e=this.copyBit(t,8,e);
	for(t=n-7;
	n>t;
...
,this.readVersion=function()
{
	...
	for(var t=0,n=e-11,i=5;
	i>=0;
	i--)for(var r=e-9;
	r>=n;
	r--)t=this.copyBit(r,i,t);
	...
	r--)for(i=e-9;
	i>=n;
	i--)t=this.copyBit(r,i,t);
	if(this.parsedVersion=Version.decodeVersionInformation(t),null!=this.parsedVersion&&this.parsedVersion.DimensionForVersion==e)return this.parsedVersion;
	throw"Error readVersion"
}
,this.readCodewords=function()
{
	var e=this.readFormatInformation(),t=this.readVersion(),n=DataMask.forReference(e.DataMask),e=this.bitMatrix.Dimension;
	n.unmaskBitMatrix(this.bitMatrix,e);
	for(var n=t.buildFunctionPattern(),i=!0,r=Array(t.TotalCodewords),o=0,s=0,a=0,h=e-1;
	h>0;
	h-=2)
{
	6==h&&h--;
	for(var c=0;
	e>c;
	c++)for(var l=i?e-1-c:c,f=0;
	2>f;
	f++)n.get_Renamed(h-f,l)||(a++,s<<=1,this.bitMatrix.get_Renamed(h-f,l)&&(s|=1),8==a&&(r[o++]=s,s=a=0));
	i^=1
}
	...
	this.unmaskBitMatrix=function(e,t)
{
	...
	n++)for(var i=0;
	t>i;
	i++)this.isMasked(n,i)&&e.flip(i,n)
}
...
{
	for(var n=new GF256Poly(this.field,e),i=Array(t),r=0;
	r<i.length;
	r++)i[r]=0;
	for(var o=!0,r=0;
	t>r;
...
if(!o)for(r=new GF256Poly(this.field,i),n=this.runEuclideanAlgorithm(this.field.buildMonomial(t,1),r,t),r=n[1],n=this.findErrorLocations(n[0]),i=this.findErrorMagnitudes(r,n,!1),r=0;
	r<n.length;
	r++)
{
...
for(var i=this.field.One,r=this.field.Zero,o=this.field.Zero,s=this.field.One;
	t.Degree>=Math.floor(n/2);
	)
{
	var a=e,h=i,c=o;
	if(e=t,i=r,o=s,e.Zero)throw"r_
{
	i-1
}
 was zero";
	for(t=a,s=this.field.Zero,r=e.getCoefficient(e.Degree),r=this.field.inverse(r);
	t.Degree>=e.Degree&&!t.Zero;
	)
{
	var a=t.Degree-e.Degree,l=this.field.multiply(t.getCoefficient(t.Degree),r),s=s.addOrSubtract(this.field.buildMonomial(a,l));
...
,this.findErrorLocations=function(e)
{
	...
	for(var n=Array(t),i=0,r=1;
	256>r&&t>i;
	r++)0==e.evaluateAt(r)&&(n[i]=this.field.inverse(r),i++);
	if(i!=t)throw"Error locator degree does not match number of roots";
	return n
}
,this.findErrorMagnitudes=function(e,t,n)
{
	for(var i=t.length,r=Array(i),o=0;
	i>o;
	o++)
{
	for(var s=this.field.inverse(t[o]),a=1,h=0;
	i>h;
	h++)o!=h&&(a=this.field.multiply(a,GF256.addOrSubtract(1,this.field.multiply(t[h],s))));
...
{
	for(this.coefficients=Array(n-i),n=0;
	n<this.coefficients.length;
	n++)this.coefficients[n]=0;
	for(n=0;
	n<this.coefficients.length;
	n++)this.coefficients[n]=t[i+n]
}
	...
	if(1==e)
{
	for(var n=e=0;
	t>n;
	n++)e=GF256.addOrSubtract(e,this.coefficients[n]);
	return e
}
for(var i=this.coefficients[0],n=1;
	t>n;
	n++)i=GF256.addOrSubtract(this.field.multiply(e,i),this.coefficients[n]);
	return i
}
...
for(var i=Array(t.length),r=t.length-n.length,o=0;
	r>o;
	o++)i[o]=t[o];
	for(o=r;
	o<t.length;
	o++)i[o]=GF256.addOrSubtract(n[o-r],t[o]);
...
,this.multiply1=function(e)
{
	...
	var t=this.coefficients,n=t.length;
	e=e.coefficients;
	for(var i=e.length,r=Array(n+i-1),o=0;
	n>o;
	o++)for(var s=t[o],a=0;
	i>a;
	a++)r[o+a]=GF256.addOrSubtract(r[o+a],this.field.multiply(s,e[a]));
...
,this.multiply2=function(e)
{
	...
	if(1==e)return this;
	for(var t=this.coefficients.length,n=Array(t),i=0;
	t>i;
	i++)n[i]=this.field.multiply(this.coefficients[i],e);
...
,this.multiplyByMonomial=function(e,t)
{
	...
	for(var n=this.coefficients.length,i=Array(n+e),r=0;
	r<i.length;
	r++)i[r]=0;
	for(r=0;
	n>r;
	r++)i[r]=this.field.multiply(this.coefficients[r],t);
...
,this.divide=function(e)
{
	...
	for(var t=this.field.Zero,n=this,i=e.getCoefficient(e.Degree),i=this.field.inverse(i);
	n.Degree>=e.Degree&&!n.Zero;
	)var r=n.Degree-e.Degree,o=this.field.multiply(n.getCoefficient(n.Degree),i),s=e.multiplyByMonomial(r,o),r=this.field.buildMonomial(r,o),t=t.addOrSubtract(r),n=n.addOrSubtract(s);
	return[t,n]
}
...
function GF256(e)
{
	...
	for(var t=1,n=0;
	256>n;
	n++)this.expTable[n]=t,t<<=1,t>=256&&(t^=e);
	for(n=0;
	255>n;
	n++)this.logTable[this.expTable[n]]=n;
...
{
	for(var r=this.image,o=qrcode.height,s=this.CrossCheckStateCount,a=e;
	a>=0&&r[t+a*qrcode.width];
	)s[2]++,a--;
	if(0>a)return 0/0;
	for(;
	a>=0&&!r[t+a*qrcode.width]&&s[1]<=n;
	)s[1]++,a--;
	if(0>a||s[1]>n)return 0/0;
	for(;
	a>=0&&r[t+a*qrcode.width]&&s[0]<=n;
	)s[0]++,a--;
	if(s[0]>n)return 0/0;
	for(a=e+1;
	o>a&&r[t+a*qrcode.width];
	)s[2]++,a++;
	if(a==o)return 0/0;
	for(;
	o>a&&!r[t+a*qrcode.width]&&s[3]<n;
	)s[3]++,a++;
	if(a==o||s[3]>=n)return 0/0;
	for(;
	o>a&&r[t+a*qrcode.width]&&s[4]<n;
	)s[4]++,a++;
...
{
	for(var r=this.image,o=qrcode.width,s=this.CrossCheckStateCount,a=e;
	a>=0&&r[a+t*qrcode.width];
	)s[2]++,a--;
	if(0>a)return 0/0;
	for(;
	a>=0&&!r[a+t*qrcode.width]&&s[1]<=n;
	)s[1]++,a--;
	if(0>a||s[1]>n)return 0/0;
	for(;
	a>=0&&r[a+t*qrcode.width]&&s[0]<=n;
	)s[0]++,a--;
	if(s[0]>n)return 0/0;
	for(a=e+1;
	o>a&&r[a+t*qrcode.width];
	)s[2]++,a++;
	if(a==o)return 0/0;
	for(;
	o>a&&!r[a+t*qrcode.width]&&s[3]<n;
	)s[3]++,a++;
	if(a==o||s[3]>=n)return 0/0;
	for(;
	o>a&&r[a+t*qrcode.width]&&s[4]<n;
	)s[4]++,a++;
...
{
	e=i/7;
	for(var i=!1,r=this.possibleCenters.length,o=0;
	r>o;
	o++)
{
	...
	if(e>3)
{
	for(var t=0,n=0;
	e>n;
	n++)t+=this.possibleCenters[n].EstimatedModuleSize;
	for(e=t/e,n=0;
	n<this.possibleCenters.length&&3<this.possibleCenters.length;
	n++)Math.abs(this.possibleCenters[n].EstimatedModuleSize-e)>.2*e&&(this.possibleCenters.remove(n),n--)
}
...
{
	for(var e=0,t=0,n=this.possibleCenters.length,i=0;
	n>i;
	i++)
{
...
if(3>e)return!1;
	for(var e=t/n,o=0,i=0;
	n>i;
	i++)r=this.possibleCenters[i],o+=Math.abs(r.EstimatedModuleSize-e);
	return.05*t>=o
}
,this.findFinderPattern=function(e)
{
	this.image=e;
	var t=qrcode.height,n=qrcode.width,i=Math.floor(3*t/(4*MAX_MODULES));
	MIN_SKIP>i&&(i=MIN_SKIP);
	for(var r=!1,o=Array(5),s=i-1;
	t>s&&!r;
	s+=i)
{
	...
	for(var a=o[4]=0,h=0;
	n>h;
	h++)if(e[h+s*qrcode.width])1==(1&a)&&a++,o[a]++;
	else if(0==(1&a))if(4==a)if(this.foundPatternCross(o))
{
...
,this.foundPatternCross=function(e)
{
	for(var t=this.moduleSize,n=t/2,i=0;
	3>i;
	i++)if(Math.abs(t-e[i])>=n)return!1;
	return!0
}
,this.crossCheckVertical=function(e,t,n,i)
{
	var r=this.image,o=qrcode.height,s=this.crossCheckStateCount;
...
s[0]=0,s[1]=0,s[2]=0;
	...
	if(s[0]>n)return 0/0;
	for(a=e+1;
	o>a&&r[t+a*qrcode.width]&&s[1]<=n;
	)s[1]++,a++;
	if(a==o||s[1]>n)return 0/0;
	for(;
	o>a&&!r[t+a*qrcode.width]&&s[2]<=n;
	)s[2]++,a++;
...
{
	e=(e[0]+e[1]+e[2])/3;
	for(var i=this.possibleCenters.length,r=0;
	i>r;
	r++)if(this.possibleCenters[r].aboutEquals(e,t,n))return new AlignmentPattern(n,t,e);
...
,this.find=function()
{
	for(var t=this.startX,r=this.height,o=t+i,s=n+(r>>1),a=[0,0,0],h=0;
	r>h;
	h++)
{
	...
	a[0]=0,a[1]=0,a[2]=0;
	for(var l=t;
	o>l&&!e[l+qrcode.width*c];
	)l++;
	for(var f=0;
	o>l;
	)
{
	if(e[l+c*qrcode.width])if(1==f)a[f]++;
	else if(2==f)
{
...
if(e<this.bitPointer+1+16)
{
	...
	for(this.blockPointer++,t=0;
	t<e-(this.bitPointer+1+8);
	t++)n+=1<<t;
...
{
	var t=0,n="",i="0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ $%*+-./:".split("");
	do if(e>1)
{
	var t=this.getNextBits(11),r=t%45,n=n+i[Math.floor(t/45)],n=n+i[r];
	e-=2
}
...
{
	var t=0,n="";
	do e>=3?(t=this.getNextBits(10),100>t&&(n+="0"),10>t&&(n+="0"),e-=3):2==e?(t=this.getNextBits(7),10>t&&(n+="0"),e-=2):1==e&&(t=this.getNextBits(4),e-=1),n+=t;
	while(e>0);
	return n
}
...
,this.getKanjiString=function(e)
{
	...
	do
{
	var t=getNextBits(13),t=(t/192<<8)+t%192,i=0,i=40956>=t+33088?t+33088:t+49472,n=n+String.fromCharCode(i);
	e--
}
while(e>0);
	return n
...
if(1!=t&&2!=t&&4!=t&&8!=t)throw"Invalid mode: "+t+" in (block:"+this.blockPointer+" bit:"+this.bitPointer+")";
	...
	switch(t)
{
	case 1:for(var t=this.getFigureString(dataLength),n=Array(t.length),i=0;
	i<t.length;
	i++)n[i]=t.charCodeAt(i);
	e.push(n);
	break;
	case 2:for(t=this.getRomanAndFigureString(dataLength),n=Array(t.length),i=0;
	i<t.length;
	i++)n[i]=t.charCodeAt(i);
	e.push(n);
	break;
	case 4:t=this.get8bitByteArray(dataLength),e.push(t);
	break;
	case 8:t=this.getKanjiString(dataLength),e.push(t)
}
...
{
	for(var i=new BitMatrix(t),r=Array(t<<1),o=0;
	t>o;
	o++)
{
	for(var s=r.length,a=o+.5,h=0;
	s>h;
	h+=2)r[h]=(h>>1)+.5,r[h+1]=a;
	...
	try
{
	...
	h+=2)
{
	var c=4*Math.floor(r[h])+4*Math.floor(r[h+1])*qrcode.width,l=e[Math.floor(r[h])+qrcode.width*Math.floor(r[h+1])];
...
,FormatInformation.doDecodeFormatInformation=function(e)
{
	...
	i++)
{
	var r=FORMAT_INFO_DECODE_LOOKUP[i],o=r[0];
	...
	s++)
{
	...
	t++)
{
	var h=a.DataCodewords,c=i.ECCodewordsPerBlock+h;
	n[o++]=new DataBlock(h,Array(c))
}

}
for(t=n[0].codewords.length,r=n.length-1;
	r>=0&&n[r].codewords.length!=t;
	)r--;
	for(r++,i=t-i.ECCodewordsPerBlock,t=a=0;
	i>t;
	t++)for(s=0;
	o>s;
	s++)n[s].codewords[t]=e[a++];
	for(s=r;
	o>s;
	s++)n[s].codewords[i]=e[a++];
	for(h=n[0].codewords.length,t=i;
	h>t;
	t++)for(s=0;
	o>s;
	s++)n[s].codewords[r>s?t:t+1]=e[a++];
	return n
}
,DataMask=
{
...
{
	for(var n=e.length,i=Array(n),r=0;
	n>r;
	r++)i[r]=255&e[r];
	n=e.length-t;
	try
...
,Decoder.decode=function(e)
{
	...
	e=t.readVersion();
	for(var n=t.readFormatInformation().ErrorCorrectionLevel,t=t.readCodewords(),t=DataBlock.getDataBlocks(t,e,n),i=0,r=0;
	r<t.Length;
	r++)i+=t[r].NumDataCodewords;
	for(var i=Array(i),o=0,s=0;
	s<t.length;
	s++)
{
	var r=t[s],a=r.Codewords,h=r.NumDataCodewords;
	for(Decoder.correctErrors(a,h),r=0;
	h>r;
	r++)i[o++]=a[r]
}
...
,qrcode=
{
	imagedata:null,width:0,height:0,qrCodeSymbol:null,debug:!1,sizeOfDataLengthInfo:[[10,9,8,8],[12,11,16,10],[14,13,16,12]],callback:null,decode:function(e)
{
...
for(n=new Detector(n).detect(),qrcode.debug&&e.putImageData(qrcode.imagedata,0,0),n=Decoder.decode(n.bits).DataByte,e="",i=0;
	i<n.length;
	i++)for(r=0;
	r<n[i].length;
	r++)e+=String.fromCharCode(n[i][r]);
...
,binarize:function(e)
{
	for(var t=Array(qrcode.width*qrcode.height),n=0;
	n<qrcode.height;
	n++)for(var i=0;
	i<qrcode.width;
	i++)
...
{
	for(var t=Math.floor(qrcode.width/4),n=Math.floor(qrcode.height/4),i=Array(4),r=0;
	4>r;
	r++)
{
	i[r]=Array(4);
...
for(e=Array(4),t=0;
	...
	r++)for(o=0;
	4>o;
	o++)e[o][r]=Math.floor((i[o][r][0]+i[o][r][1])/2);
	return e
}
,grayScaleToBitmap:function(e)
{
	for(var t=qrcode.getMiddleBrightnessPerArea(e),n=t.length,i=Math.floor(qrcode.width/n),r=Math.floor(qrcode.height/n),o=Array(qrcode.height*qrcode.width),s=0;
	n>s;
	s++)for(var a=0;
	n>a;
	a++)for(var h=0;
	...
	h++)for(var c=0;
	i>c;
	c++)o[i*a+c+(r*s+h)*qrcode.width]=e[i*a+c+(r*s+h)*qrcode.width]<t[a][s]?!0:!1;
	return o
}
,grayscale:function()
{
	for(var e=Array(qrcode.width*qrcode.height),t=0;
	t<qrcode.height;
	t++)for(var n=0;
	n<qrcode.width;
	n++)
...
var n=t(e[0],e[1]),i=t(e[1],e[2]),r=t(e[0],e[2]);
...
{
	key:"start",value:function start(cameraId,configuration,qrCodeSuccessCallback,qrCodeErrorCallback)
{
	if(!cameraId)
{
...
;
	...
	var canvasElement=this._createCanvasElement(width,height);
...
,
{
	key:"_createCanvasElement",value:function _createCanvasElement(width,height)
{
...
,
{
	key:"_createVideoElement",value:function _createVideoElement(width,height)
{
...
{
	key:"getCameras",value:function getCameras()
{
	var _this=this;
	return new Promise(function(resolve,reject)
{
	...
	if(sourceInfo.kind==='video')
{
	results.push(
{
	id:sourceInfo.id,label:sourceInfo.label
}
)
}
...
,
{
	key:"_getTimeoutFps",value:function _getTimeoutFps(fps)
{
	return 1000/fps
}
...
,
{
	key:"_log",value:function _log(message)
{
...
();
	_defineProperty(Html5Qrcode,"DEFAULT_HEIGHT",250);
